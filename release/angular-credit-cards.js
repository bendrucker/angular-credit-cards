...
var _creditcards2 = _interopRequireWildcard(_creditcards);

var _number = require('./number');

var _number2 = _interopRequireWildcard(_number);

var _ccExp$ccExpMonth$ccExpYear = require('./expiration');

var _ccExp$ccExpMonth$ccExpYear2 = _interopRequireWildcard(_ccExp$ccExpMonth$ccExpYear);

var _cvc = require('./cvc');

var _cvc2 = _interopRequireWildcard(_cvc);

'use strict';

exports['default'] = _angular2['default'].module('credit-cards', []).value('creditcards', _creditcards2['default']).directive('ccNumber', _number2['default']).directive('ccExp', _ccExp$ccExpMonth$ccExpYear2['default']).directive('ccExpMonth', _ccExp$ccExpMonth$ccExpYear.ccExpMonth).directive('ccExpYear', _ccExp$ccExpMonth$ccExpYear.ccExpYear).directive('ccCvc', _cvc2['default']).name;
    ...
    compile: function compile(element, attributes) {
      ...
      return function (scope, element, attributes, ngModel) {
        ngModel.$validators.ccCvc = function (value) {
          return _cvc.cvc.isValid(value, $parse(attributes.ccType)(scope));
        };
...
var _expiration = require('creditcards');

var _partialRight = require('ap');

'use strict';

var month = _expiration.expiration.month;
var year = _expiration.expiration.year;
var isPast = _expiration.expiration.isPast;
  ...
  return {
    ...
    compile: function compile(element, attributes) {
      ...
      return function (scope, element, attributes, _ref3) {
        ...
        ngModel.$parsers.unshift(_partialRight.partialRight(year.parse, !fullYear));
    ...
    compile: function compile($element, $attributes) {
      ...
      return function ($scope, $element, $attributes, _ref) {
        ...
        $scope.$watch($attributes.ngModel, function (number) {
          ngModel.$ccType = ccNumber.type = _card.card.type(number);
        });
        function $viewValue() {
        ...
        if (typeof $attributes.ccEagerType !== 'undefined') {
          $scope.$watch($viewValue, function eagerTypeCheck(number) {
            if (!number) {
              return;
            }number = _card.card.parse(number);
            ngModel.$ccEagerType = ccNumber.eagerType = _card.card.type(number, true);
          });
        }
        if ($attributes.ccType) {
        ...
        if (typeof $attributes.ccFormat !== 'undefined') {
          $scope.$watch($viewValue, function formatInput(input) {
            ...
            var formatted = _card.card.format(_card.card.parse(input));
        ...
        ngModel.$parsers.unshift(_card.card.parse);
        ngModel.$validators.ccNumber = function (number) {
          return _card.card.isValid(number);
        };
        ngModel.$validators.ccNumberType = function (number) {
          return _card.card.isValid(number, $parse($attributes.ccType)($scope));
        };
      };
    }
  };
